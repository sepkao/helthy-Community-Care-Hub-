      ...
      <aside className="w-64 bg-white border-r flex flex-col justify-between p-4">
        <div>
          ...
          <nav className="mt-4 space-y-1">
            <NavItem icon="📊" label="ภาพรวม (Dashboard)" href="/patients" />
            <NavItem icon="👥" label="รายชื่อผู้รับการดูแล" href="/patients" />
            <NavItem icon="📝" label="บันทึกการเยี่ยม" href="/visits" />
...
function NavItem({ icon, label, href = "#", active = false }: { icon: string, label: string,href?: string, active?: boolean }) {
  return (
    <Link 
    href={href} className={`flex items-center gap-3 p-3 rounded-xl transition ${active 
